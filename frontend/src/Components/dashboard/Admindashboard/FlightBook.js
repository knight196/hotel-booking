...
export default function FlightBook() {
    ...
    const getBooking = async () => {
      const res = await axios.get('/api/flightBook')
      setbooking(res.data.flightorder)
    }
    ...
    const deletelist = async (id) => {
      await axios.delete(`/api/flightget/${id}`)
...
<Link style={{textDecoration:'none'}} className="text-dark" to={`/api/flightid/${flight.orderId}`}>
...
<i className="fas fa-plane left-plane"></i>
    ...
    <p className={flight.Cancel === false ? 'd-none' : 'd-block text-center h3'}>Cancelled</p>
</Link>

              <div className="d-flex justify-content-between">
              <h5>User-email: {flight?.email}</h5>
